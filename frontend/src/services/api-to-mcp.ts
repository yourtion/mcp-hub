...
export class ApiToMcpService {
  ...
  ): Promise<{ id: string; message: string; config?: ApiToolConfig }> {
    const request: CreateApiConfigRequest = { config };
    const response = await api.post<
      ApiResponse<{ id: string; message: string; config?: ApiToolConfig }>
    >('/api-to-mcp/configs', request);
    return handleApiResponse(response);
  }
  ...
  async updateConfig(
    ...
    const response = await api.put<
      ApiResponse<{ id: string; message: string; config?: ApiToolConfig }>
    >(`/api-to-mcp/configs/${configId}`, request);
    return handleApiResponse(response);
  }
  ...
  async testConfig(
    ...
    const response = await api.post<TestApiConfigResponse>(
      `/api-to-mcp/configs/${configId}/test`,
      request,
    );
   ...
   */
  async importConfig(
    importConfig: ApiImportConfig,
  ): Promise<{ configs: ApiToolConfig[]; message: string }> {
  ...
  ): Promise<{ data: string; filename: string; message: string }> {
    const response = await api.post<
      ApiResponse<{ data: string; filename: string; message: string }>
    >('/api-to-mcp/configs/export', exportConfig);
    return handleApiResponse(response);
  }
  ...
  async validateConfig(
    ...
    >('/api-to-mcp/configs/validate', { config });
    return handleApiResponse(response);
  }
   ...
   * 批量测试API配置
   */
  async batchTestConfigs(configIds: string[]): Promise<{
    results: Record<string, TestApiConfigResponse>;
    message: string;
  }> {
    const response = await api.post<
      ApiResponse<{
    ...
    >('/api-to-mcp/configs/batch-test', { configIds });
    return handleApiResponse(response);
  }
