...
describe('ApiTester', () => {
  ...
  it('renders correctly', () => {
    ...
    expect(wrapper.find('.tester-header h3').text()).toBe('API测试与MCP工具预览');
  });
  ...
  it('clears test results correctly', async () => {
    ...
    const clearButton = clearButtons.find(btn => btn.text()?.includes('清除结果'));
    if (clearButton) {
  ...
  it('handles tool preview errors gracefully', async () => {
    ...
    // Mock API error
    mockApiToMcpService.generateToolPreview.mockRejectedValue(new Error('Generation Error'));
  ...
  it('removes parameters correctly', async () => {
    ...
    const removeButtons = wrapper.findAll('t-button-stub').filter(btn => 
