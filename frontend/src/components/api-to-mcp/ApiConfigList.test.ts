...
describe('ApiConfigList', () => {
  ...
  beforeEach(() => {
    ...
    router = createRouter({
      ...
      routes: [
        { path: '/api-to-mcp', name: 'ApiToMcp', component: { template: '<div></div>' } },
      ],
    });
    
    // Mock successful responses
    (apiToMcpService.getConfigs as any).mockResolvedValue({
      configs: [
        {
          id: 'test-config-1',
          name: 'Test API',
    ...
    (apiToMcpService.getConfigStats as any).mockResolvedValue({
      totalConfigs: 1,
      activeConfigs: 1,
      totalTools: 5,
      lastUpdated: '2023-01-01T00:00:00Z',
  ...
  it('handles config deletion', async () => {
    ...
    (apiToMcpService.deleteConfig as any).mockResolvedValue({
      id: 'test-config-1',
    ...
    const deleteButton = wrapper.findAll('t-button-stub').find(btn => 
  ...
  it('handles batch deletion', async () => {
    ...
    const batchDeleteButton = wrapper.findAll('t-button-stub').find(btn => 
  ...
  it('handles API errors gracefully', async () => {
    ...
    // Mock API error
    (apiToMcpService.getConfigs as any).mockRejectedValue(new Error('API Error'));
