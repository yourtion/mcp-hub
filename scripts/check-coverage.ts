...
function main() {
  ...
  if (coverageData.length > 0) {
    // 计算总体覆盖率（简单平均）
    const totalBranches = coverageData.reduce((sum, c) => sum + c.total.branches.pct, 0) / coverageData.length
    const totalFunctions = coverageData.reduce((sum, c) => sum + c.total.functions.pct, 0) / coverageData.length
    const totalLines = coverageData.reduce((sum, c) => sum + c.total.lines.pct, 0) / coverageData.length
    const totalStatements = coverageData.reduce((sum, c) => sum + c.total.statements.pct, 0) / coverageData.length
